...
export default function TipModal({ isOpen, onClose, creator }) {
  ...
  const handleSendTip = () => {
    setTipped(true);
    setTimeout(() => {
      alert(`Merci ! Ton pourboire de ${selectedAmount}€ a bien été envoyé à ${creator.name} ☕✨`);
      setTipped(false);
      onClose();
    }, 1000);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
      <div className="relative w-full max-w-sm rounded-3xl bg-[#121624] border border-amber-500/30 p-6 shadow-2xl space-y-5">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-slate-400">Offre un petit thé relaxant pour remercier ce créateur.</p>
        </div>
